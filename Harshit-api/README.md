...
This API allows users to create, view, update, and delete testimonials.

## Endpoints

- `POST /api/testimonials`: Create a new testimonial.
- `GET /api/testimonials`: Retrieve all testimonials.
- `GET /api/testimonials/:id`: Retrieve a testimonial by ID.
...
3. Set up MongoDB Atlas and configure the `.env` file with `PORT` and `MONGO_URI`.
4. Run the server: `node api/server.js`
